        ...
        if emails:
            ...
            for email in emails:
                if email not in self.emails:
                    dom=email.split('.')[-1]
                    if 'co' in dom and len(dom)==2: email+='.uk'
...
if __name__ == '__main__':
    em = ExtractEmails('https://henninglarsen.com', print_log=True, user_agent='Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:66.0) Gecko/20100101 Firefox/66.0',
                       depth=100)
    em.to_csv('henninglarsen.csv')
